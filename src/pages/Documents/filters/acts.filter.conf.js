...
export const signedEnum = {
  signed: 'signed',
  unsigned: 'unsigned',
};

export const signedEnumRu = {
  signed: 'Подписан',
    ...
    filters: [
      {
        ...
        props: {
          isMulti: false,
          options: Object.entries(signedEnumRu).map(([value, label]) => ({
            value,
            label,
          })),
        },
        decodeUrlValue: (value) => {
          ...
          return {value:decodedValue == 1 ? signedEnum.signed : signedEnum.unsigned, label:decodedValue == 1 ? signedEnumRu.signed : signedEnumRu.unsigned};
        },
        toUrlValue: (values) =>
          values
            ? Array.isArray(values)
              ? values[0].value==='signed' ? '1' : '0'
              : values.value ==='signed' ? '1' : '0'
            : '',
      },
      {
        type: 'select',
      ...
      //     })),
      //   },
      //   decodeUrlValue: (values) =>{
      //     const decodedValue = decodeURIComponent(values);
      //     return { value: decodedValue, label: periodEnumRu[decodedValue] }
      //   },
      //   toUrlValue: (values) => {
